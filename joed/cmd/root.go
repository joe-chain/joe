...
func NewRootCmd() (*cobra.Command, params.EncodingConfig) {
	...
	rootCmd := &cobra.Command{
		Use:   "joed",
		Short: "Joe it once, Joe it twice. Joe is the only thing that is very nice.",
...
func (a appCreator) appExport(
	...
	if height != -1 {
		JoeApp = app.NewJoeApp(logger, db, traceStore, false, map[int64]bool{}, homePath, uint(1), a.encCfg, app.GetEnabledProposals(), appOpts, wasmOpts)

		if err := JoeApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		JoeApp = app.NewJoeApp(logger, db, traceStore, true, map[int64]bool{}, homePath, uint(1), a.encCfg, app.GetEnabledProposals(), appOpts, wasmOpts)
	}

	return JoeApp.ExportAppStateAndValidators(forZeroHeight, jailAllowedAddrs)
}
