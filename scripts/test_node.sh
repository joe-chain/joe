#!/bin/bash
# Run this script to quickly install, setup, and run the current version of joe without docker.
#
# Example:
# CHAIN_ID="local-1" HOME_DIR="$HOME/.joe1/" TIMEOUT_COMMIT="500ms" CLEAN=true sh scripts/test_node.sh
# CHAIN_ID="local-2" HOME_DIR="~/.joe2/" CLEAN=true RPC=36657 REST=2317 PROFF=6061 P2P=36656 GRPC=8090 GRPC_WEB=8091 ROSETTA=8081 TIMEOUT_COMMIT="500ms" sh scripts/test_node.sh

export KEY="joe1"
export CHAIN_ID=${CHAIN_ID:-"local-1"}
export MONIKER="localhoe"
export KEYALGO="secp256k1"
export KEYRING=${KEYRING:-"test"}
export HOME_DIR=$(eval echo "${HOME_DIR:-"~/.joe/"}")

export BINARY=${BINARY:-joed}
...
command -v joed > /dev/null 2>&1 || { echo >&2 "joed command not found. Ensure this is setup / properly installed in your GOPATH."; exit 1; }
...
from_scratch () {
  ...
  update_test_genesis '.app_state["bank"]["params"]["send_enabled"]=[{"denom": "ujoe","enabled": true}]'
  ...
  update_test_genesis '.app_state["gov"]["deposit_params"]["min_deposit"]=[{"denom": "ujoe","amount": "1000000"}]'
  update_test_genesis '.app_state["crisis"]["constant_fee"]={"denom": "ujoe","amount": "1000"}'  

  update_test_genesis '.app_state["tokenfactory"]["params"]["denom_creation_fee"]=[{"denom":"ujoe","amount":"100"}]'

  update_test_genesis '.app_state["feeshare"]["params"]["allowed_denoms"]=["ujoe"]'

  # Allocate genesis accounts
  BINARY add-genesis-account $KEY 10000000ujoe,1000utest --keyring-backend $KEYRING
  BINARY add-genesis-account feeacc 1000000ujoe,1000utest --keyring-backend $KEYRING

  BINARY gentx $KEY 1000000ujoe --keyring-backend $KEYRING --chain-id $CHAIN_ID
...
BINARY start --pruning=nothing  --minimum-gas-prices=0ujoe --rpc.laddr="tcp://0.0.0.0:$RPC"