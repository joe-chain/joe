...
import (
	...
	appparams "github.com/reecepbcups/joe/app/params"
	"github.com/reecepbcups/joe/tests/e2e/configurer/config"
	"github.com/stretchr/testify/require"
)

func (n *NodeConfig) StoreWasmCode(wasmFile, from string) {
	n.LogActionF("storing wasm code from file %s", wasmFile)
	cmd := []string{"joed", "tx", "wasm", "store", wasmFile, fmt.Sprintf("--from=%s", from), "--gas=auto", "--gas-prices=0.1ueve", "--gas-adjustment=1.3"}
...
func (n *NodeConfig) InstantiateWasmContract(codeId, initMsg, from string) {
	...
	cmd := []string{"joed", "tx", "wasm", "instantiate", codeId, initMsg, fmt.Sprintf("--from=%s", from), "--no-admin", "--label=ratelimit"}
...
func (n *NodeConfig) WasmExecute(contract, execMsg, from string) {
	...
	cmd := []string{"joed", "tx", "wasm", "execute", contract, execMsg, fmt.Sprintf("--from=%s", from)}
	n.LogActionF(strings.Join(cmd, " "))
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) QueryParams(subspace, key string, result any) {
	cmd := []string{"joed", "query", "params", "subspace", subspace, key, "--output=json"}
...
func (n *NodeConfig) SubmitParamChangeProposal(proposalJson, from string) {
	...
	cmd := []string{"joed", "tx", "gov", "submit-proposal", "param-change", "/eve/param_change_proposal.json", fmt.Sprintf("--from=%s", from)}
...
func (n *NodeConfig) FailIBCTransfer(from, recipient, amount string) {
	...
	cmd := []string{"joed", "tx", "ibc-transfer", "transfer", "transfer", "channel-0", recipient, amount, fmt.Sprintf("--from=%s", from)}
...
func (n *NodeConfig) SubmitTextProposal(text string, initialDeposit sdk.Coin) {
	...
	cmd := []string{"joed", "tx", "gov", "submit-proposal", "--type=text", fmt.Sprintf("--title=\"%s\"", text), "--description=\"test text proposal\"", "--from=val", fmt.Sprintf("--deposit=%s", initialDeposit)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) DepositProposal(proposalNumber int, isExpedited bool) {
	...
	cmd := []string{"joed", "tx", "gov", "deposit", fmt.Sprintf("%d", proposalNumber), deposit, "--from=val"}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) VoteYesProposal(from string, proposalNumber int) {
	...
	cmd := []string{"joed", "tx", "gov", "vote", fmt.Sprintf("%d", proposalNumber), "yes", fmt.Sprintf("--from=%s", from)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) VoteNoProposal(from string, proposalNumber int) {
	...
	cmd := []string{"joed", "tx", "gov", "vote", fmt.Sprintf("%d", proposalNumber), "no", fmt.Sprintf("--from=%s", from)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) BankSend(amount string, sendAddress string, receiveAddress string) {
	...
	cmd := []string{"joed", "tx", "bank", "send", sendAddress, receiveAddress, amount, "--from=val"}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) CreateWallet(walletName string) string {
	...
	cmd := []string{"joed", "keys", "add", walletName, "--keyring-backend=test"}
...
func (n *NodeConfig) GetWallet(walletName string) string {
	...
	cmd := []string{"joed", "keys", "show", walletName, "--keyring-backend=test"}
	outBuf, _, err := n.containerManager.ExecCmd(n.t, n.Name, cmd, "")
	require.NoError(n.t, err)
	re := regexp.MustCompile("eve1(.{38})")
	walletAddr := fmt.Sprintf("%s\n", re.FindString(outBuf.String()))
...
func (n *NodeConfig) SubmitUpgradeProposal(upgradeVersion string, upgradeHeight int64, initialDeposit sdk.Coin) {
	...
	cmd := []string{"joed", "tx", "gov", "submit-proposal", "software-upgrade", upgradeVersion, fmt.Sprintf("--title=\"%s upgrade\"", upgradeVersion), "--description=\"upgrade proposal submission\"", fmt.Sprintf("--upgrade-height=%d", upgradeHeight), "--upgrade-info=\"\"", "--from=val", fmt.Sprintf("--deposit=%s", initialDeposit)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
