...
# then log output of file /code/bin/joed
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/bin/joed \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/bin/joed | grep "statically linked")
...
COPY --from=go-builder /code/bin/joed /usr/bin/joed
COPY --from=go-builder /code/bin/joed /
