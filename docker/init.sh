...
joed init $MONIKER1 --chain-id $CHAINID --home $TESTNETS_SHARED_DIR

# key1 - eve1hj5fveer5cjtn4wd6wstzugjfdxzl0xpysfwwn
echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | joed keys add $KEY --keyring-backend $KEYRING --algo $KEYALGO --home $V1 --recover
# key2 - eve1j4rtuq6zm5mmw9xcjmm7gymlj39tvwnt9h4sm2
echo "yard toss ritual ticket dirt address hood stock shiver add client sketch still brave pen win affair orphan employ choose dream sail slogan poverty" | joed keys add $KEY2 --keyring-backend $KEYRING --algo $KEYALGO --home $V2 --recover
...
joed add-genesis-account eve1hj5fveer5cjtn4wd6wstzugjfdxzl0xpysfwwn 100000000ueve --keyring-backend $KEYRING --home $V1
joed add-genesis-account eve1hj5fveer5cjtn4wd6wstzugjfdxzl0xpysfwwn 100000000ueve --keyring-backend $KEYRING --home $TESTNETS_SHARED_DIR # also add to main genesis

joed gentx $KEY 1000000ueve --keyring-backend $KEYRING --chain-id $CHAINID --home $V1 --moniker $MONIKER1 --commission-rate=0.10 --commission-max-rate=1.0 --commission-max-change-rate=0.01 --min-self-delegation "1" --ip 127.0.0.1 --node-id 017e3b0c9a050091cc2bc609af9fb861d3710215

# Key 2.
mkdir -p $V2/config
cp $TESTNETS_SHARED_DIR/config/genesis.json $V2/config/genesis.json

joed add-genesis-account eve1j4rtuq6zm5mmw9xcjmm7gymlj39tvwnt9h4sm2 100000000ueve --keyring-backend $KEYRING --home $V2
joed add-genesis-account eve1j4rtuq6zm5mmw9xcjmm7gymlj39tvwnt9h4sm2 100000000ueve --keyring-backend $KEYRING --home $TESTNETS_SHARED_DIR

joed gentx $KEY2 1000000ueve --keyring-backend $KEYRING --chain-id $CHAINID --home $V2 --moniker $MONIKER2 --commission-rate=0.10 --commission-max-rate=1.0 --commission-max-change-rate=0.01 --min-self-delegation "1" --ip 127.0.0.1 --node-id 017e3b0c9a050091cc2bc609af9fb861d3710215

# KEY1_NODE_ID=`joed tendermint show-node-id --home $V1`
# KEY2_NODE_ID=`joed tendermint show-node-id --home $V2`
...
joed collect-gentxs --gentx-dir $TESTNETS_SHARED_DIR/config/gentx --home $TESTNETS_SHARED_DIR
joed validate-genesis --home $TESTNETS_SHARED_DIR
...
# screen -dmS n1 joed start --home $V1 --moniker $MONIKER1 --address "tcp://0.0.0.0:26658" --api.address "tcp://0.0.0.0:1317" --grpc-web.address "0.0.0.0:9091" --grpc.address "0.0.0.0:9090" --p2p.laddr "tcp://127.0.0.1:26656" --rpc.laddr "tcp://127.0.0.1:26657" --proxy_app "tcp://127.0.0.1:26658" --p2p.persistent_peers "017e3b0c9a050091cc2bc609af9fb861d3710215@127.0.0.1:26666"
# echo -e "\nStarting the second node ($V2)"
# screen -dmS n2 joed start --home $V2 --moniker $MONIKER2 --address "tcp://0.0.0.0:26668" --api.address "tcp://0.0.0.0:1327" --grpc-web.address "0.0.0.0:9101" --grpc.address "0.0.0.0:9100" --p2p.laddr "tcp://127.0.0.1:26666" --rpc.laddr "tcp://127.0.0.1:26667" --proxy_app "tcp://127.0.0.1:26668" --p2p.persistent_peers "017e3b0c9a050091cc2bc609af9fb861d3710215@127.0.0.1:26656"
