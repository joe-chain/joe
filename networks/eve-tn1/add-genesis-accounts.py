...
CHAIN_ID = "eve-tn1"
GENESIS_FILE=f"{Path.home()}/.joed/config/genesis.json" # Home Dir of the genesis
FOLDER = "gentx"
...
def resetGenesisFile():    
    if not os.path.exists(GENESIS_FILE):
        print(f"# [!] Genesis file doesn't exist. Run `joed init test --chain-id {CHAIN_ID}`")
        return
...
def createGenesisAccountsCommands():
    ...
    for file in gentx_files:
        ...
        if val_addr not in CUSTOM_GENESIS_ACCOUNT_VALUES.keys():
            # print()
            output += f"joed add-genesis-account {val_addr} {amt}uexp,10000000000ucraft #{moniker}\n"
            continue # 
    ...
    for account in CUSTOM_GENESIS_ACCOUNT_VALUES:
        ...
        output += f"joed add-genesis-account {account} {CUSTOM_GENESIS_ACCOUNT_VALUES[account]}\n"
    ...
    print(f"# [!] THEN `joed collect-gentxs --gentx-dir gentx/`")
    print(f"# [!] THEN `joed validate-genesis`")
