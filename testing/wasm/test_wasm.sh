...
getCodeID() {
    TX_HASH=$1
    # joed q tx $TX_HASH --output json | jq -r '.events[] | select(.type=="store_code").attributes[0].value' | base64 --decode
    joed q tx $TX_HASH --output json | jq -r '.logs[].events[] | select(.type=="store_code").attributes[0].value'
}

echo "STORING CW721 TO CHAIN"
TX721=$(joed tx wasm store cw721_base.wasm -y --broadcast-mode block --output json $joed_COMMAND_ARGS | jq -r '.txhash')
# CODE_ID_721=$(joed query tx $TX721 --output json | jq -r '.logs[0].events[-1].attributes[0].value')
CODE_ID_721=`getCodeID $TX721`
echo "CW721 WAS STORED, WITH CODE ID $CODE_ID_721"
NFT721_TX_UPLOAD=$(joed tx wasm instantiate "$CODE_ID_721" '{"name": "joed-721","symbol": "ctest","minter": "eve1hj5fveer5cjtn4wd6wstzugjfdxzl0xpysfwwn"}' --label "base_cw_721" $joed_COMMAND_ARGS --output json --broadcast-mode block -y --admin $KEY_ADDR | jq -r '.txhash')
echo "INSTANCE instantiate'd"
ADDR721=$(joed query tx $NFT721_TX_UPLOAD --output json | jq -r '.logs[0].events[0].attributes[0].value') && echo "ADDR 721: $ADDR721"
...
function mintToken() {
    ...
    TXMINT=$(joed tx wasm execute "$CONTRACT_ADDR" "$EXECUTED_MINT_JSON" --from $KEY --yes --output json --broadcast-mode block | jq -r '.txhash') && echo $TXMINT
}
...
echo $(joed q wasm contract-state smart "$ADDR721" '{"all_nft_info":{"token_id":"1"}}' --output json) | jq -r '.data.info.token_uri'

# query all token_ids in a given contract
joed query wasm contract-state smart $ADDR721 '{"tokens":{"owner":"eve1hj5fveer5cjtn4wd6wstzugjfdxzl0xpysfwwn","start_after":"0","limit":50}}'